...
=== Making a Generic Segment Wrapper
We could create a generic `MemorySegment` wrapper that relieve us from the burden of writing custom constructors, `toString()`, `hashCode()` and `equals()` method for such classes:
...
As always, it is important to declare the `MethodHandle` instances `final`.
